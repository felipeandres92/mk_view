...
mk.controllers.Basic.prototype._Agente = function (pressed,k) {
    var m = mk.moves.types,
      f = this.fighters[1],
      leftOrient = mk.fighters.orientations.LEFT,
      rightOrient = mk.fighters.orientations.RIGHT,
      orient = f.getOrientation(),
      self = this;
	var players=mk.moves.types,
      f1 = this.fighters[0],
      leftOrient2 = mk.fighters.orientations.LEFT,
      rightOrient2 = mk.fighters.orientations.RIGHT,
      orient2 = f1.getOrientation(),
      self2 = this;
	var x1=f.getX();
	var x2=f1.getX();
	var dist=x1-x2;
	...
	if(rand==0){//pega
		...
		if(rand==0) return m.UPPERCUT;
		else if(rand==1) return m.SQUAT_LOW_KICK;
		else if(rand==2) return m.SQUAT_HIGH_KICK;
		else return m.SQUAT_LOW_PUNCH;
	    }else if(pressed[k.UP]){
	        return m.UPPERCUT;
	    }else{
		rand=Math.round(Math.random()*6);
		if(rand==0)  return m.UPPERCUT;
		else if(rand==1) return m.SPIN_KICK;
		else if(rand==2) return m.LOW_KICK;
		else if(rand==3) return m.HIGH_KICK;
		else if(rand==4) return m.LOW_PUNCH;
		else m.HIGH_PUNCH;		
	    }	   
	}else if(pressed[k.HK] || pressed[k.HP] || pressed[k.LP] || pressed[k.LK]){ //bloquea
	    	 return m.BLOCK;
	}else{// se arranca
	    rand=Math.round(Math.random()*3);
	    if(orient2==leftOrient2){
		if(rand==0)  return m.BACKWARD_JUMP;
		else if(rand==1) return m.FORWARD_JUMP;
		else  return m.WALK;
	    }else{
		if(rand==0)  return m.BACKWARD_JUMP;
		else if(rand==1) return m.FORWARD_JUMP;
		else return m.WALK_BACKWARD;
 	    }
	}	
    }else if(dist>-272 && dist <272){ // te persigue o arranca
	var rand=Math.round(Math.random()*2);
	    if(orient2==leftOrient2){
		if(rand==0)  return m.BACKWARD_JUMP;
		else return m.WALK_BACKWARD;
	    }else{
		if(rand==0)  return m.FORWARD_JUMP;
		else return m.WALK;
 	    }
    }else {// te persigue
 ...
 mk.controllers.keys.p2 = {
  ...
  mk.controllers.Multiplayer.prototype._addHandlers = function () {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      var move = self._Agente(pressed, mk.controllers.keys.p1,mk);
    ...
    document.addEventListener('keyup', function (e) {
      ...
      var move = self._getMove(pressed, mk.controllers.keys.p1, 0);
      self._moveFighter(f1, move);
	var move = self._Agente(pressed, mk.controllers.keys.p1);
      self._moveFighter(f2, move);
    }, false);
